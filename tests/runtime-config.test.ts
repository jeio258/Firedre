...
describe("runtime.getWallpaperConfig", () => {
	...
	it("homeSubtitles 为数组(middleware已解析)时直接采用", () => {
		const config = getWallpaperConfig({
			settings: { theme: { homeSubtitles: ["副标题一", "副标题二"] } },
		});
		expect(config.common?.homeText?.subtitle).toEqual(["副标题一", "副标题二"]);
	});

	it("homeSubtitles 为 JSON 字符串时解析为数组", () => {
		const config = getWallpaperConfig({
			settings: { theme: { homeSubtitles: '["副标题A","副标题B"]' } },
		});
		expect(config.common?.homeText?.subtitle).toEqual(["副标题A", "副标题B"]);
	});
