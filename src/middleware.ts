...
export const onRequest = defineMiddleware(async (context, next) => {
	...
	// 默认文章 seed（仅首次生效）：posts 表为空时导入 posts/firedre.md，非空库零开销跳过。
	// 模块级 promise 缓存保证每个 Worker 实例只检测一次；seed 幂等，并发无害。
	...
	// HTML 缓存命中检查（Workers Cache API，key 为完整 URL，含配置版本号；/admin、/api 不缓存）
	...
	if (
		...
		try {
			...
			// Cache API 的 match/put 接受 Request 或 URL 字符串，非 URL 字符串会解析失败导致缓存永不生效。
			// 用完整 URL 作 key，版本号作为查询参数携带（pathname 保留原路径，避免 URL 结构歧义）。
	...
	if (request.method === "GET") {
		...
		if (isCacheableHtml && htmlCacheKey && response.status === 200) {
			// 浏览器侧：max-age=0+must-revalidate（配置即时生效，浏览器每次回源验证）；
			// Cache API 侧：存储的 Response 带 max-age=60，供 Worker 内缓存 TTL 参考。
