...
const search = async () => {
	...
	try {
		results = await searchPostsApi(keyword);
	} catch (error) {
...
<div class="card-base px-6 py-6 md:px-9 md:py-6 mb-4 rounded-(--radius-large)">
    ...
    <div class="relative flex">
        <div class="relative flex-1">
            ...
            <input
                type="text"
                class="block w-full p-4 pl-10 text-[16px] md:text-sm bg-transparent border border-black/10 dark:border-white/10 rounded-lg focus:ring-2 focus:ring-(--primary) focus:border-(--primary) hover:border-black/20 dark:hover:border-white/20 text-75 placeholder:opacity-50 transition-colors outline-hidden"
