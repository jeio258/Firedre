...
const BUCKET = {
	...
	async put(
		key: string,
		value: string | ArrayBuffer | ArrayBufferView | Buffer,
	) {
		const p = r2Path(key);
...
const __globalCache = globalThis as unknown as {
	__firedreHtmlCache?: Map<string, Response>;
};
__globalCache.__firedreHtmlCache ??= new Map<string, Response>();
const __htmlCache = __globalCache.__firedreHtmlCache;
