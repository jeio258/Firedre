// 后台共用线性图标（feather 风格 path，单源存放），供 AdminApp / AdminDashboard 等共用
...
export function icon(name: string): string {
	return ICON_PATHS[name] || ICON_PATHS.settings;
}

export function iconSvg(name: string, size = 18): string {
	return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="${size}" height="${size}" aria-hidden="true">${icon(name)}</svg>`;
}
