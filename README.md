...
D1 数据库 `firedre-blog` 共 14 张业务表，迁移 SQL 位于 `migrations/`（本地由 `cf-dev-shim` 按文件名幂等应用，生产用 `pnpm d1:migrate`）：

| 表 | 用途 |
|---|---|
| posts | 文章元数据（正文存 R2，由 `r2_key` 指向）；`published` 标识发布状态（1=已发布，0=草稿） |
| posts_fts | 文章全文检索（FTS5 虚拟表，按 `slug` 关联 posts） |
| post_taxonomy | 分类 / 标签统一表（`type` 区分 `category` / `tag`） |
| notice_board | 公告栏（单行，`id=1`） |
| dynamics | 动态 / 说说 |
| site_settings | 站点设置强一致源（KV 镜像的权威来源） |
| rate_limits | 通用限流 + 登录失败锁定（`kind` 区分） |
| friends | 友链 |
| album_passwords | 相册访问密码（明文，SSR 端加密与解锁校验） |
| admin_users | 后台管理员凭据（`password_hash` 为 bcrypt 哈希） |
| album_webdav | 相册 WebDAV 源配置 |
| albums | 相册主表（元数据 + 正文） |
| album_photos | 相册照片表（按 `sort_order` 排序，级联删除） |
| site_links | 站点外链（导航栏 / footer / 资料卡 / 打赏，`location` + `kind` 区分） |

> 历史废弃表 `post_categories` / `post_tags` / `api_rate_limits` / `admin_login_attempts` 已在迁移 `0010_cleanup.sql` 中清理；`posts.headings_json` 冗余列亦已移除（标题由 `renderMarkdown` 渲染时实时生成）。
