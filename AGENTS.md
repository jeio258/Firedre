...
- `pnpm test`: run the vitest unit suite (tests in `tests/**/*.test.ts`).
- `pnpm test:e2e`: run the Playwright headless-browser smoke suite (`scripts/e2e-smoke.cjs`).
...
Vitest is the configured unit-test framework: suites live in `tests/**/*.test.ts` (run with `pnpm test`; watch with `pnpm test:watch`), and the Playwright headless-browser smoke suite runs with `pnpm test:e2e`. Before submitting changes, run `pnpm test`, `pnpm check`, and `pnpm type-check`; run `pnpm build` for rendering, content, or generated-asset work. For visual or interactive changes, verify with `pnpm dev` or `pnpm preview` and include screenshots in the PR. Name future tests near the feature they cover, using the local file name as the stem.
